...
def update_tracks(tracks, detections, frame_id, iou_thresh=0.3, max_age=2):
    ...
    for i, d in enumerate(detections):
        if not used[i]:
            tracks.append(TrackedObject(d['box'], d['class_id'], frame_id))
    tracks = [t for t in tracks if frame_id - t.last_seen <= max_age]
    return tracks

# ---------- Stair Detector (with temporal smoothing) ----------
class StairDetector:
    def __init__(self):
    ...
    def detect_stairs(self, depth_map):
        ...
        if is_stair:
            ...
            if len(self.direction_buffer) > 5: self.direction_buffer.pop(0)
...
# ---------- Edge Obstacle Detector (with forbidden mask) ----------
...
class OptimizedNavigation:
    ...
    def get_direction_suggestion(self, left, center, right):
        ...
        if center > STOP_CENTER:
            return "STOP", 1.0
        if center > CAUTION_CENTER:
            if left < right and left < CLEAR:
                return "Move left", 0.8
            elif right < left and right < CLEAR:
                return "Move right", 0.8
            else:
                return "Caution, obstacle ahead", 0.7
        if left > SIDE_BLOCK and right < CLEAR:
            return "Move right", 0.9
        if right > SIDE_BLOCK and left < CLEAR:
            return "Move left", 0.9
        if left < CLEAR and right < CLEAR:
            return "Go straight", 1.0
        if left < right:
            return "Slight left", 0.6
        else:
            return "Slight right", 0.6
    ...
    def process_frame(self, rgb_image, midas_model, transform):
        ...
        for t in self.tracks:
            ...
            cv2.rectangle(forbidden, (x1,y1), (x2,y2), 1, -1)
        ...
        if is_stair:
            ...
            terrain = "flat"
            warnings, annotated = self._handle_flat(rgb_image, depth, road_mask, forbidden, self.tracks)
        else:
            terrain = "irregular"
        ...
        if road_mask is not None:
            edge_contours = self.edge_obstacle.detect(rgb_image, road_mask, forbidden, scale_factor=0.5)
            cv2.drawContours(annotated, edge_contours, -1, (255,255,0), 2)
    ...
    def _handle_flat(self, rgb, depth, road_mask, forbidden, tracks):
        warnings = []
        ann = rgb.copy()
        ...
        def zone_density(mask, x1, x2):
            if x2 <= x1: return 0
            zone = mask[:, x1:x2]
            return np.sum(zone) / (zone.shape[0] * (x2-x1)) if zone.size>0 else 0

        left_d = zone_density(obstacle_mask, left_zone[0], left_zone[1])
        center_d = zone_density(obstacle_mask, center_zone[0], center_zone[1])
        right_d = zone_density(obstacle_mask, right_zone[0], right_zone[1])

        direction_cmd, conf = self.get_direction_suggestion(left_d, center_d, right_d)
        warnings.append(direction_cmd)

        # Draw info panel (non‑overlapping)
        panel_x, panel_y = 10, 100
        line_h = 25
        cv2.putText(ann, f"DIR: {direction_cmd}", (panel_x, panel_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,0),2)
        cv2.putText(ann, f"L:{left_d:.0%} C:{center_d:.0%} R:{right_d:.0%}", (panel_x, panel_y+line_h), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255),1)
...
def main():
    ...
    print("\nEnter phone IP (e.g., 192.168.1.100):")
    ip = input("> ").strip()
    ...
    if not cap.isOpened():
        print("Failed, trying USB camera...")
    ...
    print("\n=== Directional Navigation Active ===")
    print("Commands: STOP, Move left/right, Go straight, Caution")
    ...
    while True:
        ...
        if warnings and now - last_warning_time > 2.0:
            ...
            for w in warnings:
                print(f"🔊 {w}")  # Simulated voice output

        # Show warnings on screen (top left)
        for i, w in enumerate(current_warnings[:2]):
            cv2.putText(combined, w, (10, 60 + i*30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255),2)

        # Terrain color bar
        color = (0,255,0) if terrain=="flat" else ((0,0,255) if terrain=="stairs" else (0,165,255))
        cv2.rectangle(combined, (0,0), (200,30), color, -1)
        cv2.putText(combined, f"TERRAIN: {terrain.upper()}", (5,22), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0),1)

        fps = 1/(time.time()-start+1e-6)
        cv2.putText(combined, f"FPS: {fps:.1f}", (combined.shape[1]-100, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255),1)

        out_video.write(combined)
        cv2.imshow("Navigation + Depth", combined)
